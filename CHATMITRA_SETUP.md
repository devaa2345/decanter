...
- URL: your deployed bot's `/webhook` endpoint (e.g.
  ...
  locally first.
- Event types: subscribe to **`message.received`** and **`message.sent`**.
  `message.received` is the core one (`message.failed`/`message.status.updated`
  are ignored if they arrive — no need to subscribe to those). `message.sent`
  powers the human-handoff feature (see app/handoff.py): it's how the bot
  detects you personally messaging a customer directly through Chat Mitra
  (rather than through this bot), so it can pause itself for that
  conversation. Without this subscription, that feature silently never
  triggers — the bot will just keep behaving as if you hadn't sent anything.
