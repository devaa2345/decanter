...
# MIXED_EVENTS plus one first-contact welcome — used by the tests below that
# check the welcome_first_contact exclusion specifically, kept separate from
# MIXED_EVENTS so the existing counts/assertions above didn't need touching.
EVENTS_WITH_WELCOME = MIXED_EVENTS + [
    {
        "message_text": "hi",
        "perfume_id": None,
...
# MIXED_EVENTS plus one human-handoff-pause event — same reasoning as
# EVENTS_WITH_WELCOME, for the human_handoff_pause exclusion (see
# app.handoff).
EVENTS_WITH_HANDOFF = MIXED_EVENTS + [
    {
        "message_text": "sauvage",
        "perfume_id": None,
...
class TestGetOverview:
    ...
    def test_welcome_first_contact_excluded_from_unmatched(self):
        """Same reasoning as order_confirmation: a first-contact welcome is
        a distinct message type, not a failed price lookup, so it must not
        inflate 'unmatched' either (see app.formatter.WELCOME_MESSAGE)."""
        result = _run_with_events(lambda: analytics.get_overview(days=30), EVENTS_WITH_WELCOME)
        assert result["total_queries"] == 7
        assert result["welcomes"] == 1
        assert result["unmatched"] == 1  # still just the genuine "hello there" miss

    def test_handoff_pause_excluded_from_unmatched(self):
        """Same reasoning again: a query that arrived while the bot was
        paused for a human handoff is a distinct case, not a catalog gap
        (see app.handoff)."""
        result = _run_with_events(lambda: analytics.get_overview(days=30), EVENTS_WITH_HANDOFF)
        assert result["total_queries"] == 7
        assert result["handoff_pauses"] == 1
        assert result["unmatched"] == 1  # still just the genuine "hello there" miss
...
class TestGetTimeseries:
    ...
    def test_welcome_first_contact_gets_its_own_bucket_not_unmatched(self):
        result = _run_with_events(lambda: analytics.get_timeseries(days=30), EVENTS_WITH_WELCOME)
        day2 = result[1]
        assert day2["date"] == "2026-07-19"
        assert day2["welcome"] == 1
        assert day2["unmatched"] == 1  # still just the genuine "hello there" miss
        assert day2["total"] == 4

    def test_handoff_pause_gets_its_own_bucket_not_unmatched(self):
        result = _run_with_events(lambda: analytics.get_timeseries(days=30), EVENTS_WITH_HANDOFF)
        day2 = result[1]
        assert day2["date"] == "2026-07-19"
        assert day2["handoff"] == 1
        assert day2["unmatched"] == 1  # still just the genuine "hello there" miss
        assert day2["total"] == 4
...
class TestGetUnmatchedQueries:
    UNMATCHED_EVENTS = [
        ...
        {
            "message_text": "hi there first time",
            "perfume_id": None,
            "ambiguous": False,
            "layer": "welcome_first_contact",
        ...
        {
            "message_text": "sauvage while paused",
            "perfume_id": None,
            "ambiguous": False,
            "layer": "human_handoff_pause",
            "created_at": "2026-07-19T15:00:00+00:00",
        },
    ]

    def test_welcome_first_contact_excluded(self):
        """A first-contact welcome must not show up as a catalog gap either
        - it's a fixed reply to a brand-new sender, not a missed perfume
        query (see app.formatter.WELCOME_MESSAGE)."""
        ...
        assert not any("first time" in t.lower() for t in texts)

    def test_handoff_pause_excluded(self):
        """A query that arrived while the bot was paused for a human
        handoff must not show up as a catalog gap either - the bot simply
        stayed out of the conversation on purpose (see app.handoff)."""
        result = _run_with_events(
            lambda: analytics.get_unmatched_queries(days=30), self.UNMATCHED_EVENTS
        )
        texts = [g["message_text"] for g in result]
        assert not any("while paused" in t.lower() for t in texts)
