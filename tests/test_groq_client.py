...
class TestBuildSystemPrompt:
    ...
    def test_does_not_leak_the_rest_of_the_catalog(self):
        ...
        # Sanity bound: three entries should produce a small prompt, nowhere
        # near the ~23.5K-token full-catalog prompt that broke production by
        # blowing through Groq's per-minute token limit on every call. The
        # bound is generous because the fixed instructions are most of the
        # length and are expected to grow; what must stay small is the part
        # that scales with the candidate list.
...
class TestPhrasingNeverCarriesAPrice:
    """
    Prices come from catalog.py via app.formatter and from nowhere else. The
    system prompt forbids the model from writing one, and the model does it
    anyway — observed live, a follow-up about a 5ml decant came back with the
    opening "Afnan 9PM Rebel ka 5ml price hai ₹", cut off mid-number by the
    token limit, sitting directly above the real price card.
    """

    @pytest.mark.parametrize(
        "line",
        [
            "Afnan 9PM Rebel ka 5ml price hai ₹",
            "It is Rs 450 for the 5ml",
            "That one costs 500 rupees",
            "Around $60",
            "INR 1200 for this one",
        ],
    )
    def test_money_in_phrasing_is_discarded(self, line):
        assert groq_client._without_prices(line) is None

    @pytest.mark.parametrize(
        "line",
        [
            "Sauvage is one of our favourites!",
            "9PM Rebel is a great pick",
            "Baccarat Rouge 540 is lovely",
            "212 VIP, nice choice",
        ],
    )
    def test_product_names_containing_digits_are_kept(self, line):
        """Dropping anything with a number in it would throw away most valid
        openings — the catalog is full of names like "9PM Rebel", "212 VIP"
        and "Baccarat Rouge 540". Currency is the signal, not digits."""
        assert groq_client._without_prices(line) == line

    def test_none_passes_through(self):
        assert groq_client._without_prices(None) is None

    def test_applied_to_a_real_classification(self):
        payload = json.dumps(
            {
                "perfume_ids": [_REAL_PID],
                "explicit_ask": True,
                "opening": "That one is ₹450",
                "closing": "Order now!",
            }
        )
        fake, _ = _mock_openai_client(_mock_response(payload))
        with patch("app.groq_client.AsyncOpenAI", return_value=fake):
            result = asyncio.run(classify_and_phrase("sauvage", candidates=CANDIDATES))

        assert result.perfume_ids == [_REAL_PID]
        assert result.opening is None
        assert result.closing == "Order now!"
