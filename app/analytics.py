...
async def get_overview(days: int = 30) -> dict:
    ...
    # Welcome-first-contact messages aren't a failed price lookup either -
    # same exclusion reasoning as order_confirmations, just for the other
    # special-cased reply (see app.formatter.WELCOME_MESSAGE).
    welcomes = sum(1 for e in events if e.get("layer") == "welcome_first_contact")
    # A query that arrived while the bot was paused for this sender (the
    # owner already personally took over — see app.handoff) isn't a failed
    # price lookup either; same exclusion reasoning as the two above.
    handoff_pauses = sum(1 for e in events if e.get("layer") == "human_handoff_pause")
    # Order confirmations, first-contact welcomes, and handoff-paused
    # queries are distinct message types, not a failed price lookup —
    # excluded here so they don't deflate the match rate or look like
    # catalog gaps (see get_unmatched_queries).
    unmatched = total - matched - ambiguous - order_confirmations - welcomes - handoff_pauses
...
async def get_timeseries(days: int = 30) -> list[dict]:
    ...
    for e in events:
        ...
        elif e.get("layer") == "welcome_first_contact":
            bucket["welcome"] += 1
        elif e.get("layer") == "human_handoff_pause":
            bucket["handoff"] += 1
    ...
    return [
        {
            ...
            "welcome": c.get("welcome", 0),
            "handoff": c.get("handoff", 0),
            "total": sum(c.values()),
        }
...
async def get_unmatched_queries(days: int = 30, limit: int = 50) -> list[dict]:
    ...
    unmatched = [
        ...
        and e.get("layer") != "welcome_first_contact"
        and e.get("layer") != "human_handoff_pause"
    ]
